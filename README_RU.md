...
- **Своя сборка ядра на актуальном Go**: Xray-core компилируется из исходников свежим стабильным тулчейном Go, а не скачивается официальным архивом релиза. `latest` следует за самым свежим релизом Xray-core, включая prerelease; `alpha` следует за веткой `main` апстрима и пересобирается каждые 12 часов.
...
- **Fake-IP DNS по умолчанию**: на каждый DNS-запрос контейнер отдает адрес из Fake-IP пула. Этот пул можно маршрутизировать обратно в контейнер, чтобы дальнейший доступ к ресурсу осуществлялся через прокси.
...
| Тег | Назначение |
|---|---|
| `latest` | Самый свежий релиз Xray-core, включая prerelease. |
| `alpha` | Ветка `main` апстрима, пересборка каждые 12 часов. |
| `vX.Y.Z` | Конкретный релиз Xray-core. |
| `<sha>` | Конкретный коммит из `main`, по короткому SHA. |
...
| `DNS_DIRECT` | `true` | Отправляет собственные DoH-запросы контейнера напрямую, мимо прокси. `false` — резолвить через туннель; адрес самого прокси при этом резолвит системный резолвер, потому что резолвить его через туннель означало бы уже иметь туннель. |
...
- Чтобы зафиксировать конкретное ядро Xray вместо самого свежего, используйте тег версии, например `ghcr.io/medium1992/xray-proxy-ros:v26.7.28`.
- Xray-core собирается из исходников воркфлоу этого репозитория, на свежем стабильном Go и с теми же флагами сборки, что и в апстриме. Ядро сообщает о себе версию `X.Y.Z-ros` (`X.Y.Z-ros.alpha` в альфа-канале), чтобы в панели было видно, что это не официальная сборка. Готовые бинарники публикуются в релизы `stable` и `alpha` этого репозитория, откуда их и забирает Docker build.
